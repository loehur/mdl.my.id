...
<?php
		...
		if(isset($_POST['hapus_transaksi'])){
			...
			$id = $_POST['id'];
			$nama = $_POST['nama'];			
			$tsql = "select * from [transaksi_cicilan] WHERE ID = '$idt' AND Jumlah = '$jumlah'";
			$result = sqlsrv_query($conn, $tsql);
			$msg = "<h3 align=center>ERROR!</h3><p align=center>ID dan Jumlah Tidak Sesuai!<br><br></p>";
			while($row = sqlsrv_fetch_array($result))
			{
				$sql = "delete from transaksi_cicilan where ID = '$idt' AND Jumlah = '$jumlah'";
				$query = sqlsrv_query($conn, $sql);
				$ssql = "insert into kas([Tanggal],[Kategori],[Jenis],[Jumlah],[Keterangan]) values('$tanggal','Cicilan','3','-$jumlah','Penghapusan')";
