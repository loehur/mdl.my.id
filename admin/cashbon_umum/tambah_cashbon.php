...
<?php
		...
		if(isset($_POST['cashbon'])){
			...
			$nama = $_POST['nama'];
			$id = $_POST['id'];
			$keterangan_adm = $_POST['keterangan'];
				$asql = "insert into transaksi_cashbon_umum([IDC], [Jumlah], [Tanggal], [Keterangan]) values('$id','$jumlah','$tanggal','$keterangan_adm')";
				$aquery = sqlsrv_query($conn, $asql);
				if($aquery) {
					$msg = "<h3 align=center>SUKSES!<br></h3><p align=center>Penambahan/Penarikan Cashbon : <b>".number_format($jumlah)."</b> Berhasil!";
					} else {
