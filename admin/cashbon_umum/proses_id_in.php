...
<?php
		...
		date_default_timezone_set('Asia/Jakarta');
			$tanggal = date('d-m-Y');
		if(isset($_POST['p_id_in'])){
			$id_in = $_POST['id_in'];
			$id = $_POST['id'];
				$sql = "update cashbon_umum set ID_IN = '$id_in' WHERE ID = '$id'";
				$query = sqlsrv_query($conn, $sql);
				if($query) {
					$msg = "<h3 align=center>SUKSES!<br></h3><p align=center>Terdaftar di ID Investor : <b>".$id_in."</b> Berhasil!";
					} else {
